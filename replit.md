...
- **Resolution bookkeeping**: when the next turn arrives, the moves the server actually applied are derived from the board delta (`deriveLastMoves`) and fed to `applyResolvedMoves` — decision-log update and UI `move-committed` events — before the new board is applied. (Goto-queue advancement is NOT done here: arrival is detected from the board itself in `updateGameState`.)
