...
## Goto/Near Waypoint Redesign (2026-07-14)

The centaur Goto (alt-click, green) and Near (shift-click, blue) commands were redesigned from "hard path override" / "Manhattan-closeness pull" into **weighted votes in the heuristic matrix**:

1. **Per-move progress stats** (`gotoProgress` / `nearProgress`, replacing `waypointGoto` / `waypointNear`): for each candidate move, BFS shortest-path distance to the target is compared against the distance from the current head. Stat = +1 when the move is the first step of a shortest path, 0 sideways, −1 backward, −2 when it cuts the target off entirely (Near also scores −2 for landing ON the target — Near means "minimise distance without ever reaching it"). Stats are bounded so the weighted contribution can never dwarf survival terms. The shared pathfinder + stat functions live in `src/logic/waypoint-pathing.ts`; the decision engine computes the stats once per move and injects them via `EvaluationContext` (same pattern as `h2hRisk`).
2. **Configurable weights** (`gotoProgress` default 300, `nearProgress` default 250 — config.html sliders 0–1000): above food/territory pulls, below deaths (−500) and trapped (−600), so the snake follows the target but never dies for it. The old `waypointGoto`/`waypointNear` config keys are retired (stored rows are ignored after the rename — deliberate, since the old 2500/2000 magnitudes would override survival under the new stat semantics).
3. **Staging integrates the weight** (`getWaypointBiasedMove` in active-game-manager): this turn's `moveEvaluations` are re-scored with the CURRENT target's `weight × stat` (subtracting whatever waypoint contribution was recorded at `/move` time) and the winner is chosen with the same exported `pickBestMove` (trapped-veto + argmax) the engine uses. A waypoint set mid-turn therefore takes effect immediately, and the staged move is always the matrix's best output — never a blind path-follow.
4. **Two paths, tracked separately**: the optimal-next-move analysis from the current head feeds the matrix; the RENDERED green path is derived from the staged move — `[stagedDestination, ...shortestPath(stagedDestination → target)]` — recomputed on every stage/commit (`refreshGotoRoute`, stored on `ControlledSnake.gotoRoute`, NOT in the intent). The waypoint targets are the only durable state.
5. **Goto is a queue**: `{kind:'goto', targets: Coord[]}` — Shift+Alt+click appends (appending an already-queued cell removes it), reaching `targets[0]` shifts the queue, an empty queue reverts to heuristic. Near stays single-target and never auto-clears. Wire shape: `waypoints[snakeId] = {type:'green'|'blue', cells:[...]}`; intent modes `'goto'`/`'near'` replaced `'waypoint'`.

All synced surfaces updated in lockstep: config.html, history viewer stats table (board-renderer.js), decision-logger breakdown types, play-game.html (click handling, multi-target rendering with order badges, mode badges, tooltips), and `.agents/memory/intent-mode-invariant.md`.
