...
- **Write-through staging, no commit**: `ActiveGameManager.stageMove` — the single writer of every snake's staged move — publishes EVERY staging action to Firestore as a `privateMoves` write via the injected `MoveSubmitter` (bot recommendation, manual selection, queue step, waypoint step, revert-to-heuristic, kill-all). Firebase is the single source of truth for staged moves; the server-side mirror exists only for UI broadcasts. There are no `moveStatuses` commit writes and no safety timers — the game server resolves each turn with the last staged move before its own deadline. Identical (turn, move) re-stages are deduped.
