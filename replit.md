...
- **Resolution bookkeeping**: when the next turn arrives, the moves the server actually applied are derived from the board delta (`deriveLastMoves`) and fed to `applyResolvedMoves` — decision-log update and UI `move-committed` events — before the new board is applied. (Goto-queue advancement is NOT done here: arrival is detected from the board itself in `updateGameState`.) `applyResolvedMoves` also persists a per-turn **command-state snapshot** (`command_turn_states`, via `CommandLogger`) capturing every snake's command state exactly as it stood when the turn ended — staged moves, goto/near targets, routes, intent modes, owners, and the commanding operator — in the same shape the live WebSocket broadcast uses, so the history viewer replays command state through the identical render paths.
...
- **Goto / Near click-targets** — right-click sets a green "goto" target, shift+right-click cues another onto its queue, ctrl-click sets the blue "near" target (alt+left-click inspects a cell). All board input is resolved by one delegated capture-phase `pointerdown` handler on `.canvas-wrapper`, and waypoint state is server-owned with no client echo — see `.agents/memory/board-input-handling.md`. Neither overrides the snake's move: each contributes a bounded `[0,1]` shortest-path progress stat to the same heuristic matrix every other term feeds (weights `gotoProgress` / `nearProgress`, defaults 300 / 250 — below the death and trapped penalties, so survival always outvotes a target). Reaching the active goto target promotes the next in the queue; an emptied queue reverts to the heuristic. The rendered green path chains a leg per queued target (head → target 1 → target 2 → …) so the board shows the snake's whole predicted trajectory, not just the first hop — legs past the first are a prediction that assumes each target is reached, and only `targets[0]` reaches the decision engine. Near deliberately never rewards arrival. The shared pathfinder and stat functions live in `src/logic/waypoint-pathing.ts` and serve all three consumers — the evaluator's stat, the staging re-bias, and the rendered green path — so the number scored, the path drawn, and the move committed cannot disagree.
- **Command logging** — every operator command (goto set/append/remove, near, manual move, waypoint clear, fatal-move confirmation, suicide, Submit All commit) is recorded to `command_events` with the issuing operator's identity, alongside system transitions (goto arrival shifts). Each snake's `intentBy` tracks the operator whose command is in force (surviving deselects/disconnects; preserved across goto arrival shifts) and drives staged-arrow colours and replay attribution. `GET /api/logs/commands?game_id=…` returns the events plus the per-turn command-state snapshots; the `/game/:id` replay fetches it and renders recorded waypoints, goto routes, staged arrows, and owner tags with the same functions live play uses, plus a per-snake "command — operator" badge in the inspection panel.
...
- **`server_events` table** records boot, shutdown (written on SIGTERM with a bounded flush), and woke / went-idle transitions from the ActivityController's single awake rule: the instance is awake iff a **verifiable human action** happened within the last 60s, OR a **robustly-progressing game** is live AND a human action happened within the last 10 minutes (an absolute cap — no game holds the instance awake longer than that past the last human action, and a stuck game counts as inactive immediately). A connected-but-untouched browser tab counts as nothing; the only human signals are user-intent WS messages, input-gated activity-heartbeats, dashboard page loads, and mutating API calls. Three heartbeats, three jobs: the **liveness-heartbeat** (`server_liveness` DB upsert, ALWAYS on — it keeps running while idle so a silent platform kill is bounded and forensically closable), the **socket-keepalive** (WS ping/pong, connection aliveness only — never counts as activity), and the **activity-heartbeat** (browser input-gated — a human signal). Periods with no shutdown event (crash/force-kill) are closed by the next boot at the previous process's last liveness heartbeat and rendered as "end unknown".

### Database
- **PostgreSQL (Neon)** stores per-move decision logs, operator command events, and per-turn command-state snapshots for analysis and replay.
