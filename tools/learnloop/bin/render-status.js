...
/*
 * THE OWNER-FACING WORDS, WHICH ARE NOT THE SCHEMA'S WORDS.
 *
 * Owner ruling 20260830 bans "dark" and "promoted/promotion" from owner-facing
 * text and prescribes the replacements: in-collection/selectable, validated,
 * merged. `lib/ledger.js`'s seven statuses are NOT renamed — they are schema
 * constants and row identities that every historical measurement is keyed on,
 * and renaming them to change a word would rewrite the record. So the machinery
 * keeps its spelling and this view translates, which is exactly the split the
 * ruling describes: the ban is on how we WRITE to the owner, not on how the
 * loop stores its state. The table lives in the ledger (`vocabulary`) so the
 * renderer is not a second place the mapping can drift.
 */
const BADGE = {
  dark: 'merged, not selected',
...
w();
w(`Seeded ${ledger.seededAt} by ${ledger.seededBy}. ${ledger.flags.length} candidates tracked.`);
w();
if (ledger.vocabulary) {
  const v = ledger.vocabulary;
  w('## How to read the words on this page');
  w();
  w(v.ruling);
  w();
  w(`*Applies to:* ${v.appliesTo}`);
  w();
  w(`*Does not apply to:* ${v.doesNotApplyTo}`);
  w();
  w('| the ledger stores | this page says |');
  w('|---|---|');
  for (const [k, t] of Object.entries(v.translation ?? {})) w(`| \`${k}\` | ${t} |`);
  w();
  w(v.policy);
  w();
}
if (ledger.branchRoles) {
  const b = ledger.branchRoles;
  w('## The branches, and which one this record is about');
  w();
  w(b.policy);
  w();
  w('| branch | role | what it is |');
  w('|---|---|---|');
  for (const key of Object.keys(b)) {
    const r = b[key];
    if (!r || !r.branch) continue;
    w(`| \`${r.branch}\` | **${r.role}** | ${r.what} |`);
  }
  w();
  if (b.aaNull) {
    w(`**The A/A null is two builds of the \`${(b[b.aaNull.use] || {}).branch}\` bundle.** ${b.aaNull.why}`);
    w();
    w(`> ${b.aaNull.caveat}`);
    w();
  }
...
w();
w('## Where every candidate stands');
w();
...
w('## What each candidate owes, and what it has');
w();
for (const f of ledger.flags) {
  ...
  if (f.promotedBy) w(`- Validated by: ${f.promotedBy}`);
  if (f.actionOwed) w(`- **Action owed:** ${f.actionOwed}`);
  w();
  w('**Validation metrics** (the gate, and whether the build emits it):');
  w();
  ...
  if (f.distillerFinding) {
    ...
    // A FINDING THAT AN OWNER RULING SETTLED, AND WHICH HALF IT SETTLED. The
    // posed question can stop existing without the row's evidence changing, and
    // a reader who meets only `state` would still be waiting for a decision
    // that has been taken.
    if (d.resolvedBy) {
      w();
      w(`**Resolved.** ${d.resolvedBy}`);
    }
    if (d.owedRegardless) {
      w();
      w(d.owedRegardless);
  ...
  if (nx) {
    ...
    if (nx.armRefs) {
      w(`- **Cross-branch pair** — one bundle per arm, both running their shipped defaults:`);
      for (const [name, key] of Object.entries(nx.armRefs)) {
        const r = (ledger.branchRoles ?? {})[key];
        w(`  - \`${name}\` ← \`${r ? r.ref : key}\`${r ? ` (${r.role})` : ''}`);
      }
    }
