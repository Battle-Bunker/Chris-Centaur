...
async function drive(options: {
  ...
  const core: SearchCore = {
    improve: (ctx) => {
      ...
      slice++;
      const tentative = ctx.pins.find((p) => p.tentative) ?? null;
      calls.push({
        pins: [...ctx.pins],
        tentative,
...
describe('tentative pins reach the speculative machinery', () => {
  test('hovering two destinations creates a speculative context for each, searched and never emitted', async () => {
    ...
    const specSlices = run.calls.filter((c) => c.tentative !== null).length;
    expect(specSlices).toBeGreaterThan(0);
  });
  ...
  test('speculative work never leaks onto the wire (probed with a genuinely different speculative plan)', async () => {
    ...
    // Hover it, with the speculative context genuinely honouring the hover.
    const run = await drive({
      board,
      ourTeam: 'red',
      budgetMs: 250,
      bindSpeculation: true,
...
describe('adviseFromReport arithmetic (pure)', () => {
   ...
   */
  test.skip('V1-BUG-6: an unbounded speculative ceiling never reads as a free pin', () => {
    const plan: JointPlan = new Map([[4 as UnitId, cand(4, 11)]]);
    const report = {
   ...
   */
  test.skip('V1-BUG-7: advice never subtracts across constraint epochs', () => {
...
describe('BUG REPROS (V1) — tentative pins are not actually searched', () => {
   ...
   */
  test.skip('V1-BUG-5: advice brackets the true cost of a costly pin', async () => {
    const board = boardCostly();
    const [h] = handles(board, 'red', ['a', 'b']);
    const unit = h as Handle;
    // Exhaustively verified cost-10 destination (see the bench's C2 board).
    const costly = 39;
    expect(unit.dests).toContain(costly);
    const run = await drive({
      board,
      ourTeam: 'red',
      budgetMs: 250,
    ...
    expect(advice).toHaveLength(1);
    expect(advice[0]?.costLo).toBeLessThanOrEqual(10);
    expect(advice[0]?.costHi).toBeGreaterThanOrEqual(10);
  });
});
...
describe('when pin advice reaches the operator', () => {
  ...
  test('advice is delivered ONCE, after the decision loop has ended — never while it runs', async () => {
    const board = boardCostly();
    const [h] = handles(board, 'red', ['a', 'b']);
    const unit = h as Handle;
    const clock = new StepClock();
    ...
    engine.onPinAdvice(() => adviceAt.push(ports.staged.length));
    ...
    // Characterisation: nothing was delivered mid-decision. Every advice
    // callback (if any) fires only once the last staged move is already out.
    for (const at of adviceAt) expect(at).toBe(ports.staged.length);
  }, 30_000);
   ...
   */
  test.skip('V1-BUG-8: a hovered pin is priced while the decision is still running', async () => {
    const board = boardCostly();
    const clock = new StepClock();
    const ports = fakePorts(['a', 'b'], clock);
    const engine = new TeamDecisionEngine(ports, { kernel: { sliceMs: 2, reserveMs: 1, minWriteIntervalMs: 0 } });
