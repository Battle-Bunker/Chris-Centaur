...
describe('A — food control decided by the partition (seed 116 swap 0, turn 2)', () => {
  ...
  test('per-unit room reads the two roomy snakes, and neither is short of what it needs', () => {
    ...
    // held enemy's cloud is a barrier, which is what the worst world is. Its
    // `need` is 6 rather than its settled 5 because a CONTINGENT mover's weight
    // is an interval — the meal this timeline fed it is one a held enemy could
    // have taken first, and `lo` takes the largest denominator (`weightMax`,
    // see `bounds/material.ts`'s `moverWeightCeiling`).
    expect(ours.map((t) => `${t.kept}/${t.need}`)).toEqual(['5/5', '5/5', '1/6']);
  });
});
