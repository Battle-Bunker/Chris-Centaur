...
describe('BUG REPROS (V1) — asserted behaviour is the documented one; these fail today', () => {
   ...
   */
  test.skip('V1-BUG-1: an epoch re-stage is seeded from the plan the wire holds, not from nothing', async () => {
    const board = board7();
    const [h] = handles(board, 'red', ['a', 'b']);
    const unit = h as Handle;
    const run = await drive({
   ...
   */
  test.skip('V1-BUG-2: a record never claims an operator-pin it could not honour', async () => {
    const board = boardCostly();
    const [h] = handles(board, 'red', ['a', 'b']);
    const unit = h as Handle;
    const legal = new Set(unit.options.map((c) => c.to));
   ...
   */
  test.skip('V1-BUG-3: no record decided before an operator event reaches the wire after it', async () => {
    const board = boardOf(
      [
        makeSnake(
          's1',
