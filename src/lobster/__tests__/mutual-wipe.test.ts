 ...
 * from the PREVIOUS COMMITTED TURN's board, so a team ahead on weight that
 * trades its last units for everyone else's WINS. The shipped ordered clamps
 * price that world as the lattice bottom and refuse it. This file holds the
 * repair to its four guards, to the bounds laws, and — the part that matters
 * most — to the promise that with the flag OFF nothing changes at all.
 *
 ...
 *   VERDICT   the four refusals, each on a board that exercises it.
 *   CLAMPS    what the award actually does to `lo` and `hi`.
...
describe('the previous-turn verdict', () => {
  test('awards a strict lead on a fully observed board', () => {
    const sub = substrateOf([worm('a', 'red', 1, 1, 6), worm('b', 'blue', 5, 1, 5)], 'red');
    expect(mutualWipeVerdict(sub, sub.teamNumber('red'))).toBe('awarded');
  });

  test('refuses an exact tie, which the game would pay as a joint win', () => {
    const sub = substrateOf([worm('a', 'red', 1, 1, 5), worm('b', 'blue', 5, 1, 5)], 'red');
    expect(mutualWipeVerdict(sub, sub.teamNumber('red'))).toBe('not-ahead');
  });
  ...
  test('needs to beat EVERY other team, not the total and not the best of them', () => {
    ...
    expect(mutualWipeVerdict(ahead, ahead.teamNumber('red'))).toBe('awarded');
    ...
    expect(mutualWipeVerdict(level, level.teamNumber('red'))).toBe('not-ahead');
  });
  ...
  test('refuses the whole award on a board carrying an unobserved unit', () => {
    ...
    expect(mutualWipeVerdict(sub, sub.teamNumber('red'))).toBe('stale-board');
  });

  test('refuses a board with nobody to beat', () => {
    const sub = substrateOf([worm('a', 'red', 1, 1, 6)], 'red');
    expect(mutualWipeVerdict(sub, sub.teamNumber('red'))).toBe('no-rivals');
  });
  ...
  test('refuses a team with no weight on the previous board', () => {
    ...
    expect(mutualWipeVerdict(sub, 7)).toBe('no-weight');
  });
});
...
describe('what the award does to the bound', () => {
  ...
  test('a CERTAIN mutual wipe while ahead prices as a win, not as DEAD', () => {
    const sub = ahead();
    const ctx = contextOf(sub, 0, [
      { team: 0, ...GONE },
      { team: 1, ...GONE },
    ...
    expect(finish(onCtx, evaluationOf(-2, 7)).bound).toMatchObject({ lo: WIN, hi: WIN });
  });
  ...
  test('certainly gone while they only MIGHT be: the ceiling opens, the floor does not', () => {
    process.env[MUTUAL_WIPE_AWARD_ENV] = '1';
    const sub = ahead();
    ...
    // true: a mutual wipe is possible, not certain.
    const ctx = contextOf(sub, 0, [
      { team: 0, ...GONE },
      { team: 1, ...THEIRS_MAYBE },
    ]);
    const bound = finish(ctx, evaluationOf(-2, 7)).bound;
    expect(bound.lo).toBe(DEAD);
    expect(bound.hi).toBe(WIN);
  });
  ...
  test('we might survive and they are certainly gone: a win either way', () => {
    process.env[MUTUAL_WIPE_AWARD_ENV] = '1';
    const sub = ahead();
    const ctx = contextOf(sub, 0, [
      { team: 0, ...OURS_MAYBE },
    ...
    // Our contingent unit dies in the worst reading, so this IS the mutual-wipe
    // branch at `lo`, and the previous-turn lead carries it.
    expect(finish(ctx, evaluationOf(-2, 7)).bound).toMatchObject({ lo: WIN, hi: WIN });
  });
...
describe('the bounds laws under the award', () => {
  ...
  /**
   * R1, in the form the case argument makes: `lo = WIN` is claimed only where
   * every other team is gone in the reading that keeps ALL of their contingent
   * units alive — so they are gone in every consistent world, and whether we
   * survive (last team standing) or die with them (a wipe we lead), the world
   * is a win.
   */
  test('the floor only reaches WIN where every rival is certainly gone', () => {
    process.env[MUTUAL_WIPE_AWARD_ENV] = '1';
    const bits = [GONE, ALIVE, OURS_MAYBE, THEIRS_MAYBE];
    for (const ours of bits) {
      for (const theirs of bits) {
        ...
        if (lo === WIN) expect(theirs.worstAlive).toBe(false);
        clearGeometryCache();
      }
    }
  });
