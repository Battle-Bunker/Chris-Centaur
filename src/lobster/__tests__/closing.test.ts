...
/**
 * A DEFECT THIS BRANCH FOUND AND DOES NOT OWN.
 *
 * Two HELD claims that can annihilate each other break R1's CEILING in the
 * shipped fold, before any of this file's code runs. Minimal repro, 9x9, our
 * king the only mover:
 *
 ...
 */
describe('the pre-existing claim-collision ceiling gap', () => {
  ...
  it('the mechanism: only one side of a mutually fatal claim pair is flagged', () => {
...
describe('the regicide cascade prices what the rules would actually remove', () => {
  it('a maybe-regicide credits the victim team’s whole surviving roster in `hi`', () => {
    ...
    try {
      ...
      // blue's held rook (3) and knight (2) are what regicide would sweep.
      expect((cascade as { hi: number }).hi).toBeCloseTo(5, 6);
