...
/**
 * MERGE NOTE (arch/s3). S0a made `invoked` the COMPUTE gate, so a profile now
 * has to name the features it computes and `assertProfileCoherent` refuses one
 * that weights a key it does not invoke. Both i3 arms invoke EVERY key in
 * `I3_FEATURES` — which is what they did on the tree they were measured on,
 * where there was no gate and the fold ran the whole list — so the numbers
 * these arms produced are unchanged by the merge. A key that is invoked and
 * weighted zero is computed and folded as an exact zero; that is the "dark"
 * reading `I3_MATERIAL_PROFILE`'s docstring already describes for `approach`,
 * and shrinking `invoked` here to make it cheap would silently change a
 * measured arm.
 */
export const I3_FEATURE_KEYS: ReadonlySet<string> = new Set(I3_FEATURES.map((f) => f.key));

export const I3_TERRITORY_PROFILE: CriterionProfile = {
  name: 'i3-territory',
  weights: I3_WEIGHTS,
  invoked: I3_FEATURE_KEYS,
