 ...
 *
 * Nine per-feature classes are open on 240 boards at the time this file is
 * written, and closing them is nine repairs in nine terms. Asserting zero
 * would make the file red on arrival and therefore useless; asserting nothing
 * would let the next change add a tenth silently. So it is a RATCHET: each
 * class is pinned at what it measures now, a class may only go DOWN, and a
 * class not in this table may not appear at all.
 *
 * The two numbers this branch moved, measured on the same 240 boards:
 *
 *     command.hi   600 → 199   `wideFood` and `positionCertain` (features.ts)
 *     reach.lo     128 → 106   the piece contest's endpoint rule, and the tie
 *                              split gated on a claim (territory.ts)
 *
 * The rest are older and are not this repair's to close. `totalLo` is 0 and
 * stays 0: the FOLD's floor is under every world on every board here, which is
 * the property `bounds/soundness.test.ts` and the exact-reply oracle both
 * check directly. The open classes are terms whose own slack another term is
 * currently absorbing — latent, not live, and each one a floor waiting for the
 * absorbing term to move.
 */
