...
| `28-SCALARIZATION.md` | the VALUE law is a declared scalarization with a four-member family, because weighted sums provably cannot reach Pareto points off the convex hull — with the regime fork that decides when the theorem bites, and a sixth inert-weight cause whose remedy is the law rather than the number |
| `29-DSM-WEIGHTED.md` | the weighted co-change pass: the signal sharpens to 3.10x, my MODEL reading is withdrawn as a work programme rather than a structure, ACTION distributes rather than splits, and the missing abstraction is named — the candidate set has no owner |
| `30-THE-ONE-INDEX.md` | the capstone: one index adopted at six of eight appearances, with two resolutions, two scales, a fifth operation (index equality licenses TIGHTENING, the positive face my algebra missed), C59 as the admission rule beside the budget's capacity rule, and induced width refused |
| `31-EMISSION-WINDOW.md` | the emission rule re-sited: published and priced rather than gating admission, with three of my own errors closed by rules I wrote — including one my own co-change data argued against |
| `32-INDEX-INVERSION.md` | the build spec for the index inversion: five instances hoisted to one definition site, the per-coordinate operations table (the index is a PRODUCT of structures, not one lattice), Law T implemented, Law H's index-side twin (hull only across horizons), the CI that refuses a sixth instance, and three operator-lens items folded |
| `33-CANDIDATE-LIFECYCLE.md` | the missing abstraction specified: the candidate set given an owner as a value that flows rather than a package that contains, carrying the admission trace the measurement had to reconstruct |
| `34-BUILD-ORDER.md` | the one consolidated plan across all five increment lists: dependencies, what changes behaviour, what is shared with which lens, the critical path to the owner's priority, and the smallest useful subset |
| `35-CLAIMS.md` | what this design claims and how each claim is held — measured, verified against source, or assumed with the cheapest refutation named; plus the ten claims already withdrawn |
| `36-COORDINATE-CENSUS.md` | the deliberate census after two accidental zero-firing findings: 6 of 41 config coordinates ever set in an emitted spec, the socket alternative never played, treatments living in prose where the tutorial is the same syntax, the ledger keyed on deleted names, and an executable live defect — 30 of 42 leaves accept any key and any type |
| `37-REGISTRY-CENSUS.md` | the registry half, executed: four of five sockets hold one entry and vary in no slate, the resolved arity is one, the two evaluator channels partition the entries between them, the repair that composes them has never run — and the rule that a socket with one member is not a joint, drawn tightly enough to bind this design's own manifest |
