...
| **O1** | frame v1: SET (reconstructed) + pins-echo HELD + share-state FLOW context + freshness; fixed cadence; no selection (budget = ∞, items ordered by relevance) — **plus the FrameLedger from the first frame** (doc 13: propensities are degenerate at budget = ∞, but the log and its serialization path exist before any selective frame ever ships) | O0 | a harness game produces frames whose SET cardinality is non-trivial (>1 on contested boards, =1 on quiet ones — search 09's contact structure predicts which); FrameLedger rows replay byte-exact |
...
| **O3** | select + AttentionPolicy + digests with cursors + the **frustration composite** (05 §2 — MAPGEN's build-it-with-the-port doctrine; its ledger dispositions are an O0 retention item) | O1, O2 | submodularity spot-check (greedy prefix property); pinned classes always present; a seeded "coincidence" event (02 §6) costs one item, not three; on a board where a goto target is refused, the composite names the refusing stage (the guidance lens's falsifier 3, shared) |
