...
| `09-RESTRICTED-GAP-RESULT.md` | **MEASURED (v2).** is the pure argmax already optimal on our boards? | `pureDuality = 0` wherever computable — but on **two informative boards of fifteen**, so the retirement is WEAK, not final. v1's cells were computed the wrong way and are superseded. The larger finding is the reverse of v1's: on contested boards **most or all plans are REFUTED**, so the floor cannot order them — §2.3 confirmed. And the opponent usually has a **dominant column**, which is *why* the saddle exists |
...
1. ~~`restrictedGap`~~ — **DONE, doc 09 v2.** Mixing buys zero wherever the question is posed, on a thin base. What remains worth shipping is `pureDuality` + **`liveRows/rows`** + **`distinctArgminCols`** as standing rows (all exact and solver-free), plus `colSupport` when the gap is nonzero, for W-1's column pruning
