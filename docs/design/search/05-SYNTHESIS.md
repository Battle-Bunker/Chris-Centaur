...
|---|---|---|---|
| **C-T1** | TIME | **DISSOLVED, and the restatement is written below rather than owed.** The contradiction was: `stageAndGate` gate 1 makes the wire monotone in the *realised* proved floor, so a mixed draw below the maximin row is refused by `switch-floor` and the equilibrium reading is unreachable through the wire. Doc 09 retires the member that needed it — on every board that produced a column, mixing buys exactly zero — so the ratchet is no longer blocking anything anyone wants to build. **What survives is a de-jure gap: the invariant is written over the realised row, and the property it is protecting is about the decision.** | **The restatement, stated so it is not owed again.** The ratchet's obligation is *"a search that hands back a weaker promise has broken the refinement lattice"*. That obligation is about **the decision's declared value under its declared reduction**, and today's realised-row form is the correct specialisation **exactly when the reduction is point-valued and the output is an argmax** — which, post-doc-09, is the only configuration anyone is building. So: **keep the code, change the comment.** Gate 1 should say it enforces monotonicity of *the declared reduced value*, and that the realised-row test is that quantity's form under a point-valued reduction. Then the **set-valued** member (S15, which doc 09 leaves standing) inherits a stated obligation — its monotone quantity is **option-set inclusion**, `S_{t+1} ⊆ S_t` within a basis — instead of colliding with an invariant whose scope was never written down. One comment now; one line of law when arity moves |
| **C-T2** | TIME | `enumDeadline` rations the enumeration by a fixed turn fraction, and the shipped fraction is **0.35 — just over a third of the whole decision**, reserved before the anytime kernel refines anything and blind to whether the partition it found is trivial. On 88.7% of boards the enumeration's whole output is then filtered by `minHamming` as within one unit of the incumbent. Zilberstein & Russell's composition result says optimal allocation among anytime components **requires each component's performance profile**, and that local compilation gives global optimality for a wide class of structures — which is the property that makes a market work rather than merely look tidy | make it a bidder whose bid is a **performance profile**, not a scalar. A scalar cannot express "I improve fast for 40 ms then flatten", which is the shape the enumeration almost certainly has. **Two asks: does the market's bid type carry a profile? and does it admit a bidder whose bid depends on a cheap prefix of its own work (`partitionOf` is cheap, `enumerateProposals` is not)?** Doc 07 §3 supplies the y-axis |
| **C-T8** | TIME | **prefix determinism** — *"a bigger budget's decision sequence is an EXTENSION of a smaller one's"* — is maintained at real cost across **six** sites (the multi-start's forfeited elite pool, peeked draws, one-temperature-read-per-slice, the enumeration's turn-fraction ration, the session-lived scout ledger, the once-computed offer order), and their counting-cut redesign is a seventh investment in it. It is nowhere named, has no property test, and its single consumer — the anytime curve — is the analysis the critic audit discounted (quantized past readability, one budget column dropped, one arm's curve invalidated by a caching artifact) | **Law I: invariants are addressed like members.** Name it once, list its sites, give it a property test (the testkit's `countingBudget` already makes this cheap), and attribute it to a live consumer. Then relaxing it *at one site* becomes measurable rather than frightening — starting with the multi-start pool, where the price is paid in search quality rather than in machinery |
...
| **S0** | **`restrictedGap`** — **RUN; see doc 09 v2.** Retain the cells the bank already computes, partition by basis, and emit `pureDuality` (exact, solver-free) plus **`liveRows/rows`** and **`distinctArgminCols`**, flagging `cols < 2` and `liveRows < 2` as structural zeros. Solve for `vMixed` only when `pureDuality > 0` | no | **three useful answers from one build.** `rowSupport = 1` on most decisions retires the whole mixed/equilibrium direction on evidence with zero games; a multi-unit gap prices it; and `colSupport ≪ cols` unlocks W-1's column pruning regardless of what the gap says. Also yields `P*` for C-B5 |
