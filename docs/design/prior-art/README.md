...
| 43 | response to the index inversion — the missing termination column, the stronger Law T, and a rider that detects but does not correct | `43-response-to-the-index-inversion.md` |
| **42** | **abstract interpretation — the bounds bank's third formal identity**, the ladder's bottom rung, and the reduced product we already hold both halves of | `42-abstract-interpretation-and-the-bounds-bank.md` |
| **41** | **multi-player search — the pruning wall**, why the member with the WRONG model wins, and the precondition our share currency already supplies | `41-multi-player-search-and-the-pruning-wall.md` |
| **40** | **minimax PATHOLOGY — "deeper is better" is a hypothesis with a checkable condition**, and our own measurement is evidence it fails where the game is decided | `40-minimax-pathology-and-the-depth-hypothesis.md` |
...
options.** Minimax pathology calls it *node-value independence* (Beal's assumption
4, and the resolution five groups reached independently); real-time heuristic
search finds it in the heuristic's error structure over neighbouring states;
determinization calls it **leaf correlation `lc`** — *"the probability all sibling,
terminal nodes have the same payoff value"*. High correlation: search helps and the
evaluator's noise averages out. Low: the selection operator picks the noise and more
search makes it worse. **Our own value lens has measured it without naming it** — a
**9.7× discontinuity** in |residual| between structurally adjacent plans at the
king-present boundary. Three consequences: **one measurement serves all three
domains** (correlate structurally adjacent plans' values, stratified by the
mechanism indicators already derived); it is a property of **the game and the
evaluator, not the algorithm**, so it belongs in the instance space as a per-cell
column; and it carries its own remedy direction, because low correlation means
**evaluator work out-returns search work in that stratum**. See domains 40, 45, 46.
...
| **C95** | BELIEF / SEARCH | **Strategy fusion is the error the hypothesis market is exposed to, and its two triggering conditions are already computable.** A search that conditions on a specific enemy configuration and optimises within it *"incorrectly believes it can use a different strategy in each world"*. The error needs **both** anti-correlated option values across hypotheses **and** a guaranteed-better option elsewhere — without the second it over-estimates but still chooses correctly. Both are available from the bank: the sign pattern of an option's value across open hypotheses, and *"does any plan's worst case beat every other plan's best case"* — **the sound floor's dominance test**. **Flag a decision fusion-exposed when a ranking flips sign across hypotheses and no option dominates on the floor.** Scope to the **advised** channel: the floor's `min` over hypotheses is already αμ's "same move in all worlds" discipline. |
| **C92** | TIME / SEARCH | **Pathology's dominant cause is ON-POLICY LEARNING, and we are on-policy by design.** **57.7% of problems pathological with learning vs 4.3% without** — a factor of thirteen. The bounds carry, ADVANCE carries, attention carries, the market carries, the memo persists: every one is learning concentrated on the trajectory actually taken, and a different rung would have taken a different trajectory. A **second, mechanically different route into pathology** requiring nothing about the evaluator's error structure — and the route our central efficiency claim is most exposed to. Quantified: the **volume of updates falls with depth (4.1 → 1.4)**, so comparing rungs compares two different amounts of accumulated learning. |
| **C93** | TIME | **Pathology's third cause IS our commitment rule, and the CPP is comparing the wrong thing.** A depth-`d` search run every `d` moves generates `off(d)/d` states per move — **deeper lookahead with longer commitment is the same compute rearranged, not more compute.** Their control is decisive: **searching every move instead of every `d` moves cut pathology from 57.7% to 13.1%**, node counts at `d=10` rising 146.3 → ~1,550. So **the CPP must compare rungs at equal TOTAL WORK across the committed span**, not at equal per-decision depth — a **second, independent argument for C68's work-unit denominator**, from a completely different direction. |
| **C94** | TIME / OWNER | **The best FIXED lookahead depth was 1, and per-instance selection was worth 38%** (175.4 → 107.9). If that shape holds even directionally, **"buy depth" is the wrong verb** — the economy's job is to *allocate depth differentially*, which is what premise conditioning is for and what a single saturation point cannot express. **And that 38% is a VBS−SBS gap for depth**: replay each archived decision at every rung and compare per-decision-best to best-single-rung. Large ⟹ conditioning is where the value is; small ⟹ a fixed rung is fine and the allocation machinery is overhead. Note also **per-move adaptation was cheaper as well as better** (nodes/move 59.3 → 34.0). |
| **C89** | BOTH OPERATOR LENSES | **The surface's damage scales with its QUALITY, and the design has no term for it.** Automation bias produces **commission** errors (following the aid *against* information the operator holds — **5.2% of cases switched from correct to incorrect** in the reviewed studies) and **omission** errors (missing what the aid did not flag). Both are mediated by trust, and trust is earned by accuracy, so **the surface's net value is not monotone in the advisor's quality**. The omission channel is **invisible by construction** — nothing logs a non-surfacing — and C88's randomised holdout is the only instrument that sees it. **One instrument, three purposes: propensity correction, threshold identification, omission detection.** |
| **C90** | OPERATOR-OUTBOUND | **The ask threshold is a PORTFOLIO constraint, not a per-decision optimum.** At **PPV 0.3, operators ignored about half of the TRUE alarms** on the difficult targets — the cases the surface exists for. Lowering the threshold lowers the surfaced set's PPV, degrading response to the alarms that matter, and the cost lands on **future** interactions where a per-decision expected-utility calculation structurally cannot see it. **Hold the surfaced set's PPV above a floor and optimise utilities within that constraint** — the outbound lens's first hard, measurable acceptance criterion. **`disuse` is the endgame if it is got wrong**: an over-alerting surface is switched off, not ignored. |
| **C91** | OPERATOR-OUTBOUND | **Two channels, two causes, two acceptance criteria.** *False alarms damage **compliance*** (acting when we signal); *misses damage **reliance*** (trusting our silence). The silent region is a signal too, and its failures have a different cause and victim: surfaced region judged by **PPV**, silent region by **the miss rate in the silence** — which only the holdout can estimate. **A single "trust"/"confidence" scalar cannot carry both**; raising the threshold improves compliance and worsens reliance, and that trade *is* the content of the middle band's width. |
| **C86/C87** | COMPOSITION | **The index operations table is missing a TERMINATION column, and Law T specifies the DIRECT product.** A widening exists to guarantee a fixpoint terminates in a domain with **infinite ascending chains** — so flat `config.*`, bounded `observable.horizon` and per-turn `support.*` need **none**, while `observable.provenance`, `measure.range` and **the value interval itself** do: a small, per-coordinate obligation. And `BOUND_RELATIVE_EPSILON` is about to be **promoted** out of the bank into the shared index module by X4, still undocumented as the widening it is. Separately, Law T's `lo=max, hi=min` is the *direct* product; the **reduced** product is strictly stronger and **d7's V-3 parity bound (a congruence) × the cell-count bound (an interval)** is the textbook instance — `[3,5]` vs `[4,4]`. **X4 is the moment**, because it fixes `tighten`'s signature for every caller. |
| **C88** | OPERATOR / MEASUREMENT | **The ratification rider detects but does not correct**, and this is **closed-loop feedback / presentation bias** — the central methodological problem of every deployed ranker. Stratification answers *is this corpus contaminated*; **correction needs the exposure probability** (inverse propensity scoring — a requirement on the *surfacing code* to log its probability), and **identification needs a small randomised holdout**, because IPS is undefined where `P(surfaced)=0`. Timing: once the surface ships **almost every row becomes a caused row**, so the refusal binds on everything unless uncaused rows are deliberately supplied. **The holdout must be designed in with the surface.** (And it corrects my own M101: fitting the ask/act threshold on overrides without the propensity log re-fits the threshold to itself.) |
| **C83** | SEARCH / COMPOSITION | **The bank is an INTERVAL abstract interpreter, and the value model above it is RELATIONAL.** `cluster-enum.ts` computes an explicitly pairwise surrogate `Σφ_u + ½Σφ_uv`; the bound travelling with it is `[worst,best]` per option — **non-relational by construction** — so the bank cannot represent the couplings the search is organised around. The field's domain for exactly this regime is the **octagon** (`±x ± y ≤ c`, i.e. *"A's gain plus B's gain ≤ c"*), at `O(n²)` memory / `O(n³)` time — applied **per unit-contribution, not per plan**, which is the granularity the surrogate already uses. Trigger to look for: bounds much looser than the surrogate's own pairwise term implies (that is d23's dependency problem, fixable by construction rather than by tuning). |
| **C84** | SEARCH | **`tighten` is the DIRECT product, and we hold both halves of the textbook case where the REDUCED product is strictly stronger.** "Floor rises to the better floor, ceiling falls to the better ceiling" keeps each answer as-is; the reduced product lets each domain **refine** the other first. Intervals × congruences is the canonical illustration — and **d7's V-3 checkerboard parity bound is a congruence while the cell-count bound is an interval**. Direct product of `[3,5]` with "even" reports `[3,5]`; reduced reports `[4,4]`. Generalised: when a second structurally different bound arrives, ask not only *is it tighter* but **does it let the others get tighter**. |
| **C85** | SEARCH / TIME | **`BOUND_EPSILON` is a WIDENING, and a widening exists to guarantee a fixpoint iteration terminates.** Within a turn, monotone tightening over a finite option set converges; **across turns, re-base and carry make it a genuine loop and nobody has argued it terminates** — and if it converges only because each pass relaxes by an epsilon, a constant introduced as a rounding fix is load-bearing for termination. Purest R-6. Plus a C11-style collision: **`withNarrowing` is not narrowing** (which recovers precision a widening discarded). |
| **C80** | SEARCH / COMPOSITION | **C22 is a THEOREM, not a bug: deep pruning is provably impossible in MaxN.** Only *shallow* alpha-beta pruning is available, and only given a sum bound — so "interval dominance is sound at the leaf and unsound propagated up the deep channel" is the **correct behaviour of the algorithm REDUCTION selected**, not an arithmetic defect. Stop looking for the arithmetic; accept shallow-only, or change the member. **The cost is not marginal**: average-case asymptotic branching factor `b` against a two-player search's near-`√b` roughly **squares the node count for a given depth**, which with the 512 joint cap already binding on 5–6-unit components is the difference between binding sometimes and binding always. |
| **C81** | SEARCH / REDUCTION | **The member whose model is WRONG wins, for our own reason.** *"Paranoid widely outperforms maxn in Chinese Checkers, by a lesser amount in Hearts, and they are evenly matched in Spades"* — paranoid's assumption is false and it wins because pruning buys depth. So the member question is not *which model of the opponents is truer* but **which buys more depth per unit of modelling error** — the sound floor's own discipline, applied to the search operator, with the field's measurement behind it. Domain-dependent, so **classify ours**: does third-team behaviour correlate with harming the leader? |
| **C82** | SEARCH / REDUCTION | **The kingmaker: a third independent argument for paranoid.** In a three-team game a team can decide another's victory without affecting its own standing — and **MaxN's prediction there is arbitrary**, because its score is unaffected either way so the choice falls to a tie-break. Paranoid at least makes a *defined and safe* prediction. The three-team balance bug is this hazard's symptom; R-1's reframing is right but incomplete — **neither canonical member can express the kingmaker case, and only one fails safely**. |
| **C77** | ALL / TIME / VALUE | **"Deeper is better" is a HYPOTHESIS with a stated condition, and it guards the flagship.** Under **independence between sibling values**, minimax *amplifies* the evaluator's error and deeper search produces **worse** decisions (Beal's own result, which he called *"disappointing"*); the resolution, reached independently by five groups, is **dependence between sibling values — the similarity of nearby positions**. And our own measurement is evidence the condition fails where it matters: the value lens measured a **9.7× step** in \|residual\| at the king-present boundary (`corr = +0.954`), and a 9.7× discontinuity between structurally adjacent plans **is** low sibling dependence. **So the wipe-closure defect is also a pathology risk, and it lives in the cells where the game is decided.** R-6's most expensive instance. |
| **C79** | TIME | **C48's second horn gets a theorem.** Pearl: to overcome pathology *the evaluator's error must decrease **exponentially** with depth* — and the field's own expectation is that evaluators do not vary that fast. So "evaluator too coarse for depth to bite on" is a **quantitative condition with a rate**, and a flattening profile is its signature. Discriminator set, with C66: *stopped changing its mind* ⟹ exhausted · *keeps flipping at a stable score* ⟹ coarse · *residual not falling with depth* ⟹ coarse, rate measured. |
| **C78** | SEARCH / REDUCTION | **MaxN has a pathology theorem named after it** (Mutchler 1991 extends Nau's to the multi-player backup). Not a disqualification — the **failure direction ruling 49 requires a member to carry**. If a depth sweep shows depth helping under `paranoid` and not under `MaxN`, that is documented behaviour, not noise. |
...
| **M116** | `[+]` **Our DISAMBIGUATION FACTOR is high — one of the survey's very few findings that predicts the architecture will WORK.** `df` is how fast the information set shrinks; the poles are trick-taking games (fast, determinization strong) and poker (slow, weak), and we are structurally near the trick-taking pole — units move and are seen, and the conditioning ladder's rungs *are* disambiguation events. **Measure it: plot `|information set|` against turn number on the archive.** Fast decay confirms the favourable regime; slow decay says we are in poker's regime and determinization needs d12's heavier machinery. |
| **M117/M118** | **`bias` is d26's dead-cell criterion derived from THEORY** — extreme bias makes large homogeneous regions every competent algorithm finds, so an extreme-bias cell cannot discriminate between arms. That gives the detector a *predictor* (estimable from a handful of games, not a full arm matrix), and we have already measured that bias varies (**0.427 → 0.530 from spawn geometry alone**). Testable: extreme-bias cells are the ones where the arm matrix is flat. And **αμ is R-4's SEVENTH arrival, the first that is specifically an imperfect-information REMEDY** — it fixes strategy fusion by playing the same moves in all valid worlds and non-locality by using **Pareto fronts as state evaluations, combined at min and max nodes**. A scalar cannot represent *"good under hypothesis A, bad under B"*; a front can. **R-4 is now a soundness requirement for search under fog**, not only a Centaur-surface argument. |
| **M114/M115** | **We already hold a depression map, computed for another purpose — and the instance space already makes per-instance depth selection affordable.** A *heuristic depression* is a bounded region where the heuristic is systematically inaccurate and an LRTA*-style agent gets trapped paying corrections one increment at a time; **d31 §31.5's king-present cells (|residual| 1.946 vs 0.201, corr +0.954) are exactly that**. The response is a *behaviour* — `mark-and-avoid`, `move-to-border` — needing no evaluator improvement, defensible as an **interim** measure with the hazard held in view: **mark-and-avoid on a decisive region is a policy, not a search heuristic**. And they made per-instance depth selection tractable by computing the optimal depth per **abstract** state — 0.004% of state pairs. Our abstraction is the **cell**, already the CPP's key, so it reduces to **one optimal-rung value per cell**. |
| **M112/M113** | **Intermediate automation preserves the take-back C73 makes mandatory — and it is M100's THIRD independent argument.** *"The out-of-the-loop performance problem was significantly greater under **full** automation than under intermediate levels"*, and at lower levels operators *"were more able to assume manual control when needed"*. Composed with C73: **the strategy must end in take-back by the agent, and the operator's ability to take back decreases with how much was automated in between.** So M100's coarser commitment and C74's buy-time action are both intermediate-automation moves — one design move satisfying three independent constraints. And **complacency's stated precondition is our standing condition**: it occurs under **multiple-task load**, appears in **experts**, and **cannot be overcome with practice** — so "they will learn to check" is not a mitigation. C76's two proxies (which unit last got a manual command; elapsed fraction of the deadline) are **exactly** those preconditions, so one coordinate serves both. |
| **M110/M111** | **Trace partitioning is C37's fifth coordinate — and it makes the coordinate CHEAPER than C37 implied.** Static analysis has the same defect (merging at joins loses what the path knew) and the same fix, with two transferable refinements: the coordinate must be a **bounded abstraction of the history**, not the history (the engineering is entirely in choosing it), and partitioning is **parameterised per site**, so carry the range coordinate *where the memo is hot and the histories differ* and merge elsewhere rather than adding a coordinate to every key. **Third independent field to reach C37's conclusion.** And: **soundness is a PER-OPERATOR obligation and our operator set is five** (`backupMax`, `backupMin`, `tighten`, the epsilon relaxation, `withNarrowing`) — discharge one obligation each and every composite is covered, which also forces M109's assertion as a by-product. |
| **M107** | **The share currency supplies the precondition multi-player pruning REQUIRES, and nobody has said so.** Korf's shallow pruning needs *an upper bound on the sum of evaluations* and *a lower bound on each*; in the share currency both are **identities** (`sharePar = K·w/W` sums to a constant, and is non-negative). So **pricing in shares is not an accounting convenience — it is the condition under which multi-player pruning is licensed at all.** This upgrades the currency from "inert accounting" (d32) to *enabling the search*, because it acts on **what gets computed** rather than on the ordering of what is computed. And it makes constant-sum a property to preserve deliberately — any later term that breaks the sum silently removes the licence. **R-6: assert `Σ shares = K`.** |
| **M108/M109** | **Speculative pruning is a member with a stated failure direction and our game qualifies** (*"the first multi-player pruning algorithm that can prune any constant-sum multi-player game"*, at the cost of possibly-wrong answers with a speculation parameter) — and it fits unusually well because the bank already separates a **sound** reading from an **advised** one, so it belongs in the advised path while the sound path keeps shallow-only. And: **the member choice is not a module, it is the min-node operator** — `backupMin` over enemies *jointly* is paranoid, per-enemy maximisation is MaxN. The pruning licence, deep-propagation soundness, kingmaker behaviour and pathology exposure **all follow from which operator sits there**, and none is stated where it is written. One comment carries four results — the highest-leverage R-6 assertion the survey has found. |
| **M103** | **The depth test is cheaper than the depth work it informs, and runs on the archive.** (a) Correlate structurally adjacent plans' evaluations **stratified by the mechanism indicators d31 already derived** — low within-stratum correlation is the pathology precondition and predicts which cells depth should hurt on; (b) re-run archived positions at increasing rungs and record whether the chosen plan's **realised outcome** improves *per cell*, because pathology is about decision quality and a root-value comparison cannot see it. If depth helps on some strata and hurts on others, the spend rule should be **conditioned on the stratum** — which is what the CPP's premise coordinate is for. |
| **M104/M105** | **A point in our favour, measurable today — and a third argument against the caps.** Uniform branching factor is the *pathological* family; non-uniform is not. Ours is extremely non-uniform (~71 options for a queen, 3 for a trail unit) and the distribution is a by-product of admission, so plot it. **But `sliderCandidateCap: 4` and `enumCandidateCap: 8` make branching MORE uniform** — if non-uniformity is the protection, the caps remove it (third argument beside C2 and M39). And pathology supplies **C24's mechanism**: minimax does not inherit evaluator error, it **selects for it**, more strongly with depth wherever siblings are independent — so evaluator work pays **super-linearly in the low-dependence strata**, which are the wipe-closure cells. Fixing the wipe closure is a pricing fix, a coverage fix and a depth fix at once. |
