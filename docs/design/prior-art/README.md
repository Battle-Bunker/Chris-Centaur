...
| **C13** | VALUE / TIME | The correct fold may be **un-incrementalisable**: `(K/W)(1−p)` is a whole-board coefficient, so one unit's commitment perturbs every unit's term. NNUE's whole lesson is that the feature basis is chosen jointly with the update path. The two lenses are optimising against each other and neither has noticed. |
