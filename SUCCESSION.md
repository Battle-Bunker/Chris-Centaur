...
## Addendum, 2026-09-05 09:00Z (read this block first; it supersedes the ones below where they differ)
...
**State.** Bot `stable/one-engine-lens-v6` = 29d308b (97 suites / 1674 tests; zero inversions on the sixteen runner arms; exact-reply oracle exact; byte-identical to v5 in play). TacticToes `stable/one-engine-v5` = 38c60f1. PR #17 and #24 bodies current. All three bot simplification plans and both TacticToes plans executed. Both correctness reviews fully worked (docs/design/REVIEW-1.md; TacticToes docs/REVIEW-1.md): six bot bugs and four engine bugs fixed with tests, the rest closed with evidence or measured and declined.

**Behaviour, the honest summary.** Two audits on one 26-game corpus (`BEHAVIOUR-AUDIT.md`, `BEHAVIOUR-AUDIT-2.md`). Taken: D1, the contest member's honest floor (edge deaths 3→0, mixed 10→6, potions 26→21). Refuted with the mechanism written beside the code and NOT to be re-derived: D2 and P1 (pawn parking: freeing the pawn buys body or contest deaths), D3 (room's fear saturates), P3 (hunger in meals over-eats into dead ends), σ (a boolean staging charge decides every floor tie alike and raises deaths). The potion member is CLOSED after five measured shapes (`potions.md`, `potion-shape.md`): the audit's target is provably unreachable on this corpus; leave `window.ts`'s potion half alone until the game changes. Class A of `contest-gap.md` (20 of 28 contest deaths: the one-ply field is silent at the entry turn) is the one open behaviour question; a design worker is on it; a null result there means class A is the price of a crowded board.

**What a successor should do next, in order.** (1) Read classA-design's verdict (branch `classA-design` or docs/design/contest-classA.md) and implement only if it names a gradient with a falsifier. (2) Rebase `feature/drives-preferences` after #17 merges. (3) The owner's decisions: TacticToes deploy hazard (docs/REVIEW-1.md §2.1) and plan-2 item 4 (lobby). (4) A third behaviour audit only with MORE BOARD (twenty seeds, or more potions and units) — the current corpus is exhausted.

**Process rules that cost us when broken.** Never `git stash` in a worktree. Every worker checkpoints and pushes after each step (container restarts and the hourly session rate limit kill all workers at once). A dead agent cannot be resumed by message; relaunch from its branch. Sonnet briefs must say "never pass run_in_background, never call Monitor, never wait for a notification". A full-suite result recorded under runner-heavy workers is not evidence; re-run failures alone. A behaviour change is kept only if it is at least as good as the baseline on every board class at full length, deaths first; a 30-turn arm is a truncation, not a class. A refuted rule earns a paragraph beside the code, not a scaffold: no fixture tests, unused builders or diagnostics for a rule that was reverted. After a selective merge, read `git diff --stat HEAD~1 HEAD` before pushing. Read `date -u` before judging a ping late.
