...
Any change to bot data structures (heuristics, field names) must be reflected across ALL synchronized surfaces in the same change: core logic (board-evaluator, decision-engine, voronoi-strategy), decision-logger types, and the config UI. The viewer's per-heuristic breakdown surface no longer exists (deleted 2026-09-01, `docs/REFACTORING.md`) — do not re-add one. The full surface-by-surface checklist lives in [combat-invulnerability-precedence.md](combat-invulnerability-precedence.md) ("Adding any new scoring heuristic") and [clearance-model.md](clearance-model.md) ("How to apply").
...
Automated browser testing (the Replit run_test tool) has been unreliable in this project — do not depend on it. For any UI-affecting change to the game/history viewer, ask the user for manual verification (screenshots of the viewer with a recent game loaded: board, candidate cells, territory overlay) before declaring success.
