...
**Starvation-aware body vacating:** health loss is movement-tied (snakes always move → −1/turn unless they eat). Phase 3 of the build: for every snake (chess pieces excluded — `(unitType ?? 'snake') !== 'snake'` units can stand still and never starve), a walls-only BFS (ignoring ALL bodies and hazards — a generous lower bound e on its earliest possible eat of already-spawned food) decides certain starvation iff `e > h` (h = health; eating ON turn h saves it). A certain starver's ENTIRE body vacates from arrival turn `max(h+1, 2)` in the optimistic and physical layers (min with the per-segment tail projection); conservative keeps its usual +1 buffer. The subjective layer never applies it to the subject's OWN body. Same accepted new-food-spawn risk class as the tail projections. Tests: `starvation-vacate.test.ts`.
